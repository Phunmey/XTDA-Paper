...
def activation_discovery(dataset, edges_asdf, graphindicator, unique_graphindicator):
   ...
   # total_degree = {}
    ...
    # plt.bar(total_degree.keys(), total_degree.values(), 1, color='b')  # 1 represents width
    ...
    # plt.title(dataset)
    # plt.savefig("/home/taiwo/projects/def-cakcora/taiwo/results3/degree_dist_figures/" + dataset + "DegreeStats.png")
...
def kernelize_graph(unique_graphindicator, edges_asdf, filtr_range, filtration, dataset, graphindicator, progress,
    ...
    for graphid in unique_graphindicator:
        ...
        eigen_value, eigen_vector = np.linalg.eig(ck_transform)
...
def train_test_rf(Param_Grid, dataset, g_test, g_train, num_cv, y_test, y_train, time_taken, file, h_filt, filtration):
    # start training
...
def main(h_filt, filtration, max_allowed_filtration):
    ...
    train_test_rf(Param_Grid, dataset, g_test, g_train, num_cv, y_test, y_train, time_taken, file, h_filt, filtration)
...
if __name__ == '__main__':
    ...
    outputFile = "/home/taiwo/projects/def-cakcora/taiwo/results3/" + 'Kernel_CTDAEvector.csv'
